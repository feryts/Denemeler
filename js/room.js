...
const Room = (function () {
  ...
  function renderInfo() {
    ...
    document.getElementById("agencyStrip").innerHTML = `
      <div class="aName">${UI.esc(room.name)} ${room.locked ? "🔒" : ""}</div>
      <div class="aSub">ID:${room.id} · Host: ${host ? UI.esc(host.username) : "?"}</div>`;

    const listeners = room.mics.filter(m => m.userId).length;
    document.getElementById("roomSub").innerHTML = `
      <span class="lvlPill">🏆 Lv.${room.level} · ${fmtNum((host && host.diamond) || 0)}</span>
      <span class="timerPill">👥 ${listeners}/12</span>`;

    document.getElementById("settingsBtn").style.display = isMod() ? "flex" : "none";

    const top = room.mics.filter(m => m.userId).map(m => DB.getUser(m.userId)).filter(Boolean)
      .sort((a, b) => b.diamond - a.diamond).slice(0, 3);
    document.getElementById("rankStrip").innerHTML = top.length ? `
      <span>🔥 Top:</span>
      ${top.map(u => `<span class="rPill">${u.avatar} ${UI.esc(u.username)} · 💎${fmtNum(u.diamond)}</span>`).join("")}
    ` : "";
  }
  ...
  function renderMics() {
    document.getElementById("micGrid").innerHTML = room.mics.map((m, i) => {
      ...
      const u = m.userId ? DB.getUser(m.userId) : null;
      const cls = ["micSeat"];
      ...
      return `
        <div class="${cls.join(" ")}" onclick="Room.tapSeat(${i})">
          <div class="seatCircle">
            ${m.locked && !m.userId ? `<span class="lockIconBadge">🔒</span>` : icon}
            ${role ? `<span class="roleTag">${role}</span>` : ""}
            ${m.muted ? `<span class="muteTag">🔇</span>` : ""}
          </div>
          ${u ? `<span class="scorePill">${fmtNum(u.diamond)}</span>` : ""}
